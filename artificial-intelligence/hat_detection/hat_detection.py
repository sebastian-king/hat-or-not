...
def get_embedding(model, img_bytes):
    ...
    #img = tf.keras.utils.array_to_img(img_bytes)

    # img to Array
    x   = tf.keras.utils.img_to_array(img_bytes)
    # Expand Dim (1, w, h)
...
def hatDetection(args, img):
	# generation of a dictionary of (title, images)
	df = pd.read_csv(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data_hat.csv'))

	# # https://scikit-learn.org/stable/modules/generated/sklearn.metrics.pairwise_distances.html
	from sklearn.metrics.pairwise import pairwise_distances

	df_embs = pd.read_csv(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'embeddings_hat.csv'))
